 ...
 * Popover that adapts:
 * - Desktop: Protrudes to the right of the sidebar for preferences.
 * - Mobile: Centered modal with backdrop blur to remain completely visible.
 */
...
export default function PreferencesPopover({
  ...
  // Modo Mobile: Modal centrado con fondo desenfocado
  if (isMobile && typeof document !== "undefined") {
    return createPortal(
      ...
      >
        ...
        <div className="relative z-10 w-full max-w-sm max-h-[85vh] overflow-y-auto bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200/90 dark:border-zinc-800 shadow-2xl p-4 animate-in fade-in zoom-in-95 duration-150">
          {content}
        </div>
      </div>,
      document.body,
  ...
  // Modo Desktop: Popover anclado sobresaliendo a la derecha del botón
  return (
    <>
      <div
        ...
        aria-hidden="true"
      />
      <div className="absolute left-[calc(100%+8px)] top-0 z-50 w-80 bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-2xl p-4 animate-in fade-in zoom-in-95 duration-150">
        {content}
      </div>
    </>
  );
